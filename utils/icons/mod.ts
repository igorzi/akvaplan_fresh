...
//`https://fonts.gstatic.com/s/i/short-term/release/materialsymbolsoutlined/arrow_back_ios/wght300/48px.svg;
...
export const svgMap = async (
  path = "/icon",
) => {
  const svg = new Map<string, string>();
  for await (const name of icons) {
    const url = [path, name + ".svg"].join("/");
    console.log(url);
    const r = await fetch(url);
    if (r.ok) {
      svg.set(name, await r.text());
    }
  }
  return svg;
};
